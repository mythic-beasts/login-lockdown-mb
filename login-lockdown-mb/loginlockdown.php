<?php
/* 
Plugin Name: Login LockDown MB
Plugin URI: https://github.com/mythic-beasts/login-lockdown-mb
Version: v1.7
Author: Michael VanDeMar, Paul Warren
Description: Adds some extra security to WordPress by restricting the rate at which failed logins can be re-attempted from a given IP range. Originally developed by Michael VanDeMar.  IPv6 support added by Mythic Beasts.
*/
...
*
* ver. 1.7 2-Sep-2016
* - forked by Mythic Beasts, adding IPv6 support
*
...
function countFails($username = "") {
  ...
  if (isV6($ip)) {
    $net = v6AddressToNetwork($ip);
  }
  else {
    $net = substr ($ip, 0 , strrpos ( $ip, "." ));
  }
	...
	$numFailsquery = $wpdb->prepare( $numFailsquery, $net  . "%");
...
function incrementFails($username = "") {
	...
	global $loginlockdownOptions;
	$table_name = $wpdb->prefix . "login_fails";
	$ip = expandV6Address($_SERVER['REMOTE_ADDR']);
...
/*
 * Expand occurrences of "::" in v6 addresses.  Safe to call on v4 addresses.
 */
function expandV6Address($ip) {
  if (strpos($ip, '::') !== false) {
...
/*
 * Extract the /64 that the v6 address exists in.
 */
function v6AddressToNetwork($ip) {
  $ip = expandV6Address($ip);
  $o = 0;
  for ($i = 0; $i < 4; $i++) {
    $o = strpos($ip, ":", $o) + 1;
  }
...
function isLockedDown() {
  ...
  if (!isV6($ip)) {
    ...
    $subnet = v6AddressToNetwork($ip);
  }

	$stillLockedquery = "SELECT user_id FROM $table_name " . 
					"WHERE release_date > now() AND " . 
					"lockdown_IP LIKE %s";
	$stillLockedquery = $wpdb->prepare($stillLockedquery,$subnet . "%");
